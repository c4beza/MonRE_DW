...
-- check if there are duplicate records in MonRE.Address
...
OR advert_id <= 0;
-- check if there are null and illegal values in MonRE.Agent
SELECT * 
FROM MonRE.Agent
...
-- check if there are null and illegal values in MonRE.Client
SELECT * 
FROM MonRE.Client
WHERE person_id IS NULL
...
OR office_id <= 0;
-- check if there are duplicate records in MonRE.Person
...
OR person_id <= 0
OR address_id <= 0;
-- check if there are records in MonRE.Person not in MonRE.Address
SELECT * FROM MonRE.Person
...
GROUP BY postcode
HAVING COUNT(*) > 1;
-- check if there are illegal and null values in MonRE.Property
SELECT * 
FROM MonRE.Property
...
OR property_size < 0;
-- check if there are duplicate records in MonRE.Property
...
HAVING COUNT(*) > 1;
-- check if there are illegal and null values in MonRE.Rent
SELECT * 
FROM MonRE.Rent
...
HAVING COUNT(*) > 1;
-- check if there are illegal and null values in MonRE.Sale
SELECT * 
FROM MonRE.Sale
...
OR client_person_id NOT IN (SELECT person_id from MonRE.Person)
OR property_id NOT IN (SELECT property_id from MonRE.Property);
-- check if there are illegal and null values in MonRE.State
SELECT * 
FROM MonRE.State
